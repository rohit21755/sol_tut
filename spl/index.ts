import { createInitializeMint2Instruction, createMint, getMinimumBalanceForRentExemptMint, getOrCreateAssociatedTokenAccount, MINT_SIZE, mintTo, TOKEN_2022_PROGRAM_ID, TOKEN_PROGRAM_ID } from "@solana/spl-token";
...
async function main() {
    const lamports = await getMinimumBalanceForRentExemptMint(connetion)
    ...
    const  sx = await sendAndConfirmTransaction(connetion, tx, [payer, mintAccount])
    console.log(sx)
